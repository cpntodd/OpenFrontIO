...
describe("toWebSocketOrigin", () => {
  it("converts production HTTPS API origins to WSS", () => {
    expect(toWebSocketOrigin("https://api.openfront.io")).toBe(
      "wss://api.openfront.io",
    );
  });

  it("converts local HTTP API origins to WS", () => {
    expect(toWebSocketOrigin("http://localhost:8787")).toBe(
      "ws://localhost:8787",
    );
  });

  it("leaves existing WebSocket origins unchanged", () => {
    expect(toWebSocketOrigin("wss://matchmaking.test/path")).toBe(
      "wss://matchmaking.test",
    );
  });
});
